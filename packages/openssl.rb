...
class Openssl < Package
  ...
  version '1.1.1g'
  source_url 'https://github.com/openssl/openssl/archive/OpenSSL_1_1_1g.zip'
  source_sha256 '41bac751d85f89a7d821324b7ffb35526a310db014ab6a4fe17fddaa011b7024'
  ...
  def self.build
    ...
    FileUtils.mkdir_p("#{CREW_DEST_DIR}/etc/ssl")
    FileUtils.mkdir_p("#{CREW_DEST_PREFIX}")

    # Specify armv7 for aarch64 since Chrome OS aarch64 uses armv7 binaries as its userland.
    case ARCH
    when 'aarch64'
      system "./Configure --prefix=#{CREW_PREFIX} --openssldir=#{CREW_DEST_DIR}/etc/ssl #{options} linux-armv4 -march=armv7-a"
    when 'x86_64'
      system "./config --prefix=#{CREW_PREFIX} --libdir=lib64 --openssldir=#{CREW_DEST_DIR}/etc/ssl #{options}"
    else
      system "./config --prefix=#{CREW_PREFIX} --openssldir=#{CREW_DEST_DIR}/etc/ssl #{options}"
    end
    system 'make'
  end
  ...
  def self.install
    ...
    FileUtils.mkdir_p("#{CREW_DEST_PREFIX}/local")
    ...
    FileUtils.mkdir_p("#{CREW_DEST_DIR}/etc")
